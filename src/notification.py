...
class Notification:
    ...
    def delete(self) -> None:
        ...
        self.bot.deleteMessage(
            chat_id=self.telegram_chat_id, message_id=self.telegram_message_id
        )
    ...
    def send(self) -> None:
        if config.TELEGRAM_ENABLE_SENDING:
            ...
            if msg["message_id"]:
                ...
                self.thread = Timer(
                    self.delete_delay, delete_notification_after_delay, [self]
                )
                self.thread.start()
        else:
            log(f"sending telegram - {self.message}")
