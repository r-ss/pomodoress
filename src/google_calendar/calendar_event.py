...
class CalendarDayHelper:
    ...
    def parse_event_text(self, event):
        ...
        if 'commute' in event.text:
            delta = get_delta(re.search(',?\s((?P<TypeDescription>commute)(?:\s)((?P<TypeTime>\d{1,2}:\d{1,2})|(?P<TypeNumber>\d)))', event.text).groupdict())
        ...
        if 'back' in event.text:
            delta = get_delta(re.search(',?\s((?P<TypeDescription>back)(?:\s)((?P<TypeTime>\d{1,2}:\d{1,2})|(?P<TypeNumber>\d)))', event.text).groupdict())
